...
class MyProcessing():
    ...
    def __init__(self,
        ...
        self.apd_path = apd_path
        self.extwav_path = extwav_path
        self.combwav_path = combwav_path
    ...
    def append_directory(self, apd_path):
        ...
        for x in self.DateID_list:
            ...
            if sys.platform == 'win32':
                fullpath = f'{file.full_path(self.apd_path, self.year, self.month)[0]}\\{x[0:4]}'
    ...
    def asf_to_wav(self):
        ...
        for x,y in zip(self.DateID_list, self.fulrwpath_list):
            ...
            dest_path = file.full_path(self.extwav_path, self.year, self.month, x)[0]
            os.chdir(y)
    ...
    def wav_combine(self):
        ...
        for x in self.DateID_list:
            ...
            source_list=[]
            raw_path = file.full_path(self.extwav_path, self.year, self.month, x)[0]
            dest_path = file.full_path(self.combwav_path, self.year, self.month, x)[0]
            os.chdir(raw_path)
            ...
            if self.check_file(dest_path, 'wav'):
                ...
                for num in num_list:
                    ...
                    if sys.platform == 'win32':
                        full_path = f'{raw_path}\\00{num}.wav'
    ...
    def wav_to_mel(self):
        ...
        for x in self.DateID_list:
            ...
            source_path = file.full_path(self.trans_path, self.year, self.month, x)[0]
            os.chdir(source_path)
            for files in os.listdir():
                # Check whether file is in wav or not
                if files.endswith(".wav"):

                    if self.check_file(self.mel_path,f'{x[5:]}.png'):

                        source = f'{source_path}/{files}'
                        destination = f'{self.mel_path}/{x[5:]}.png'
                        print(source)
                        print(destination)
        ...
        for x in self.DateID_list:
            ...
            for files in os.listdir():
                ...
                if files.endswith(".wav"):
                    ...
                    if self.check_file(self.mfcc_path, f'{x[5:]}.png'):

                        source = f'{source_path}/{files}'
                        destination = f'{self.mfcc_path}/{x[5:]}.png'
                        print(source)
                        print(destination)
